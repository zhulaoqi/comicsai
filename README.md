...
- 定时内容生成：每天 `02:00`
- 定时内容发布：每天 `06:00`
- 支持多 AI Provider（OpenAI / Qwen 文本与图像）
...
flowchart LR
    ...
    B --> P[AI Providers<br/>OpenAI / Qwen]
    ...
    click P "./backend/src/main/resources/application.yml" "查看 AI Provider 配置"
```
...
```mermaid
flowchart TD
    SJ[定时任务 Scheduler] --> G[02:00 内容生成]
    G --> C[内容草稿/审核中]
    C --> A[管理端审核]
    A --> P[06:00 自动发布]
    P --> R[用户端可见]

    click SJ "./backend/src/main/java/com/comicsai/scheduler" "查看调度任务代码"
    click G "./backend/src/main/java/com/comicsai/scheduler/ContentGenerationJob.java" "查看生成任务"
    click C "./backend/src/main/java/com/comicsai/controller/admin/ContentManageController.java" "查看内容管理接口"
    click A "./admin-portal/src/views" "查看管理端页面"
    click P "./backend/src/main/java/com/comicsai/scheduler/ContentPublishingJob.java" "查看发布任务"
    click R "./reader-app/src/views" "查看用户端页面"
```
